...
class PoseDataset(data.Dataset):
    def __init__(self, mode, num_pt, add_noise, root, noise_trans, refine):
        if mode == 'train':
            self.path = 'datasets/ycb/dataset_config/train_data_list_0and1.txt'
        elif mode == 'test':
    ...
    def dpt_2_pcld(self, depth, cam_cx, cam_cy, cam_fx, cam_fy, cam_scale):
        if len(depth.shape) > 2:
            depth = depth[:, :, 0]

        depth = depth.astype(np.float32) / cam_scale
        msk = (depth > 1e-8).astype(np.float32)

        row = (self.ymap - cam_cy) * depth / cam_fy
        col = (self.xmap - cam_cx) * depth / cam_fx

        dpt_3d = np.concatenate(
            (row[..., None], col[..., None], depth[..., None]), axis=2
        )
